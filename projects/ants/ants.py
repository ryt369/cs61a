...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        count=0
        while check_place :  
            if not check_place.is_hive:
                if check_place.bees and self.upper_bound>=count>=self.lower_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # 首先调用父类的 reduce_health 方法
        super().reduce_health(amount)  # 调用父类的 reduce_health 方法，减少健康值
        
        # 计算反射性伤害：当前伤害 + 如果死亡，增加的额外伤害
        total_damage = amount + (self.damage if self.health <= 0 else 0)
        
        # 对所有 Bees 造成反射性伤害
        for bee in list(self.place.bees):  # 复制 list，避免修改原列表时产生问题
