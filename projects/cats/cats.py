...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    list1=[x for x in paragraphs if select(x)]
    if k>=len(list1):
        return ''
    else:
        return list1[k]
    ...
    if len(typed_words)==0 and len(source_words)==0:
        ...
        for i in range(len(typed_words)):
            if typed_words[i]==source_words[i]:
                correct_corresponding +=1
        return correct_corresponding/len(typed_words)*100
    elif len(typed_words)>=len(source_words):
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    mini_difference=limit
    mini_difference_word=typed_word
    
    if typed_word in word_list:
        return typed_word
    else:
        for word in word_list:
            if diff_function(typed_word,word,limit)==limit and mini_difference_word==typed_word:
                mini_difference_word=word
            elif diff_function(typed_word,word,limit)<mini_difference:
                mini_difference_word=word
                mini_difference=diff_function(typed_word,word,limit)
            
        return mini_difference_word
...
def minimum_mewtations(typed: str, source: str, limit: int) -> int:
    ...
    if limit<0:
        return limit+1
    elif source=="" or typed=="":
        return len(source)+len(typed)
    elif typed[0]==source[0]:
        return minimum_mewtations(typed[1:],source[1:],limit)
    #分三种情况讨论 add ，remove ，substitute，取最小
    elif typed[0]!= source[0]:
...
def report_progress(typed: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    for index in range(0,len(typed)):
        if typed[index]==source[index]:
            ...
            break
    correct_rate=correct_number/len(source)
    def print_progress(id=user_id,rate=correct_rate):
        print('ID:',id, 'Progress:',rate)
    print_progress()
    return correct_rate
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    # BEGIN PROBLEM 9
    #firstly compute the first player's time
    # times=len(timestamps_per_player)*     $$$$$$[[]]不能这样写，会导致创建的多个list一起被更改
    times = []  # This will store a list for each player
    
    for player_timestamps in timestamps_per_player:
        # compute time differences for this player
        player_times = []
...
def fastest_words(game: dict) -> list[list[str]]:
    ...
    # Initialize the result with an empty list for each player
    fastest = [[] for _ in range(num_players)]
    
    # Loop through each word
    for j in range(num_words):
        # Start by assuming player 0 typed this word the fastest
        min_time = get_time(times, 0, j)
        min_player = 0
        
        # Compare times for each player
        for i in range(1, num_players):
            player_time = get_time(times, i, j)
            if player_time < min_time:
                min_time = player_time
                min_player = i
        
        # Append the word to the fastest player
        fastest[min_player].append(words[j])
