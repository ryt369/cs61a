...
def make_onion(f, g):
    ...
    def can_reach(x, y, limit):
        if limit < 0:         #base case1 输入·不和要求
            return False
        elif x == y:          #base case2 最后的情况，二者把x变到了和y相等，证明可以
            return True 
        else:
            return can_reach(f(x), y, limit - 1) or can_reach(g(x), y, limit - 1)
            #递归部分注意要传入f（x）和g（x）以实现对传入函数的调用
    return can_reach
...
def ten_pairs(n):
    ...
    def pair(n):
        ...
        if n<10:
            return 0
        elif count_digit(n//10,10-last)!=0:
            count += count_digit(n//10,10-last)
        return count+pair(n//10)
    return pair(n)
