...
class Mint:
    ...
    def __init__(self):
        self.update()
        # self.year=2025   不需要这个，update会在init调用的时候自动初始化year的值
        #这样写反而使得year无法更新
...
class Coin:
    cents = 0 # will be provided by subclasses, but not by Coin itself
    ...
    def worth(self) -> int:
        ...
        if Mint.present_year-self.year>50:
            return self.cents+(Mint.present_year-50-self.year)
        else:
            return self.cents
