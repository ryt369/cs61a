...
class Account:
    ...
    def time_to_retire(self, amount: float) -> int:
        ...
        "*** YOUR CODE HERE ***"
        self.future_money=self.balance
        count=0
        while self.future_money<amount:
            count+=1
            self.future_money=(self.interest+1)*self.future_money   
        return count
...
def without(s: Link, i: int) -> Link:
    ...
    if s is Link.empty:       
        return s
    if i == 0:                # 跳过 index = 0
        return without(s.rest, -1)  # -1 表示之后不删除任何东西
    if i > 0:                 # 还没到要删的位置
        return Link(s.first, without(s.rest, i - 1))
    else:                     # i < 0，表示已经删过了，不再删
