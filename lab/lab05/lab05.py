...
def insert_items(s: list[int], before: int, after: int) -> list[int]:
    ...
    while index<len1:
        ...
        index +=1
    return s
#注意：while循环才能改变循环次数，for循环一经开始，循环次数就改不了了。
#像这种会改变列表长度的情况，用while不用for！！!
...
def repeated(t: Iterator[int], k: int) -> int:
    ...
    while  not find:
        ...
        if count==k-1:
            return current
        current,next1=next1,next(t)
    
v = iter([4, 1, 6, 6, 7, 7, 8, 8, 2, 2, 2, 5])



def multi_func(f,x,k):
    '''return f(f(f(x)))..... for k times'''
    i=0
    assert k>0,"the k must be a positive number"
    if k==1:
        return f(x)
    while i<k:
        result=f(multi_func(f,x,k-1))
        return result
def mul(x):
    return x*x      
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        # If it's a leaf, create new branches for each leaf label
        return tree(label(t), [tree(leaf) for leaf in leaves])
    else:
        # If it's not a leaf, recursively process all branches
...
def prune_leaves(t, vals):
    ...
    if is_leaf(t):
        ...
        new_branch=[b for b in new_branch if b != None]
...
def balanced(t):
    ...
    if is_leaf(t):
        ...
        standard=sum_tree(branch_list[0])
        if not all([sum_tree(b)==standard for b in branch_list]):
            return False
        if not all([balanced(b)==True for b in branch_list]):    #关键在于检查，每一种情况，
            return False                                         #不要提前返回导致后面的情况没被检查
        return True
...
def partial_reverse(s: list[int], start: int) -> None:
    ...
    #如果剩余的元素数量是奇数
    if rest%2==1:
        while s[start1] !=s[end]:
            s[start1],s[end]=s[end],s[start1]
            end=end-1
            start1=start1+1
    #当剩余的元素的数字是偶数时
    else:
        ...
        for i in range(times):
            s[start+i],s[len1-i-1]=s[len1-i-1],s[start+i]
    return None
